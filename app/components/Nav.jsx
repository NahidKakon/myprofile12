...
import { MenuIcon, Rotate3D, X } from 'lucide-react';
...
function Nav({ className }) {
  ...
  const renderLinks = () => {
    return navLinks.map(({ id, label }) => (
      <li
        key={id}
        className="text-1.4rem font-semibold text-gray-900 transition cursor-pointer hover:text-orange-500"
    ...
    >
      <div className="flex items-center justify-between max-w-6xl mx-auto">
        ...
        <div className="items-center hidden gap-6 md:flex ">
          ...
          {/* <Button className="text-black-700 bg-orange-400 hover:bg-orange-600">
            Download CV
